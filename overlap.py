...
countOverlapCost,bins = np.histogram(data, bins)
data = np.true_divide(countOverlapCost, len(costOfOverlap))
print(data)
overlapSumData = list()
for i in range(0,len(data)):
    overlapSumData.append(sum(data[0:i+1]))
print(overlapSumData)

data = costOfBeOverlapped
bins = np.linspace(-1000, 8000, num=30, endpoint=True, retstep=False, dtype=None)
countBeOverlappedCost,bins = np.histogram(data, bins)
data = np.true_divide(countBeOverlappedCost, len(costOfBeOverlapped))
overlappedSumData = list()
for i in range(0,len(data)):
    overlappedSumData.append(sum(data[0:i+1]))
x = np.arange(-1000, 7700, 300)
plt.scatter(x, data)
plt.title("F1 Shanghai: Time cost of being overtaken from 2012 to 2018") 
plt.xlabel('wight')
plt.ylabel('numbers') 
plt.show()
